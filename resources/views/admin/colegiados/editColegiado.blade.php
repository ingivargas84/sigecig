...
@section('content')
<form method="POST" id="EditColegiadoForm" action="{{route('colegiado.update', $codigo)}}">
  {{csrf_field()}} 
  <div class="col-md-12">
      ...
      <div class="box-body">
        ...
        <div class="row">
          <div class="col-sm-4">
            ...
            <input id="dpi" autofocus="" value="{{$query->registro}}" class="form-control" name="dpi" type="text">                            
            <input type="hidden" name="num" value="{{$query->id}}">
          </div>
          <div class="col-sm-4">
            <label for="nombre">Nombres</label>
            <input id="nombres" class="form-control" value="{{$query->nombres}}" name="nombres" type="text">                            
          </div>
          <div class="col-sm-4">
        ...
        <div class="row">
          <div class='col-sm-2'>
            ...
            <select class="form-control" id="sexo" name="sexo">
              @foreach($sx as $se)
              @if ($se->c_sexo==$codigo->sexo)
              <option value="{{$se->c_sexo}}" selected>{{$se->n_sexo}}</option>
              @else
              <option value="{{$se->c_sexo}}">{{$se->n_sexo}}</option>
              @endif
              @endforeach
            </select>                            
          </div>
          <div class="col-sm-2">
            ...
            <input id="fechaNacimiento" class="form-control" name="fechaNacimiento" value="{{date('Y-m-d', strtotime($query->fecha_nac))}}" type="date" max="9999-12-31">                            
          </div>
          <div class='col-sm-4'>
            ...
            <input type="hidden" id="idMunicipioNacimiento" name="idMunicipioNacimiento" value="{{$query->c_mpo}}">
          </div>
          <div class='col-sm-4'>
            ...
            <input type="hidden" id="idDepartamentoNacimiento" name="idDepartamentoNacimiento" value="{{$query->c_depto}}">
          </div>
        </div>
        <br>
        <div class="row">
          <div class='col-sm-4'>
            ...
            <input type="hidden" id="idPais" name="idPais" value="{{$query->c_pais}}">
          </div>
          <div class="col-sm-4">
            ...
            <input type="hidden" id="idNacionalidad" name="idNacionalidad" value="{{$query->nacionalidad}}">
          </div>
          <div class="col-sm-4">
        ...
        <div class="row">
          <div class="col-sm-4">
            ...
            <input id="email" class="form-control" value="{{$query->e_mail}}" name="email" type="text">                            
          </div>
          <div class="col-sm-2">
          ...
          <div class="col-sm-2">
            ...
            <select onchange="mostrarConyugue();" id="estadoCivil" class="form-control" name="estadoCivil">
              @foreach($ecivil as $estado)
              @if ($estado->c_civil==$codigo->e_civil)
              <option value="{{$estado->c_civil}}" selected>{{ $estado->n_civil}}</option>
              @else
              <option value="{{$estado->c_civil}}">{{$estado->n_civil}}</option>
              @endif
              @endforeach
            </select>                            
          </div>
          <div class="col-sm-4">
            ...
            <input id="direccion" class="form-control" value="{{$query->direccion}}" name="direccion" type="text">                            
          </div>
        </div>
        <br>
        <div class="row">
          ...
          <div class="col-sm-3">
            ...
            <input type="hidden" id="idMunicipio" name="idMunicipio" value="{{$query->c_mpocasa}}">
          </div>
          <div class="col-sm-3">
            ...
            <input type="hidden" id="idDepartamento" name="idDepartamento" value="{{$query->c_deptocasa}}">
          </div>
          <div class="col-sm-3">
            <label for="destino" class="control-label">Destino correo</label>
            <select class="form-control" id="destino" name="destino" type="text">
              @foreach($dest as $de)
              @if ($de->destino==$codigo->destino_correo)
              <option value="{{$de->destino}}" selected>{{ $de->destino}}</option>
              @else
        ...
        <div class="row">
          <div class="col-sm-3">
            ...
            <input id="direccionTrabajo" class="form-control" value="{{$query->dir_trabajo}}" name="direccionTrabajo" type="text">                            
          </div>
          <div class="col-sm-1">
            <label for="zonaTrabajo" class="control-label">Zona</label>
            <input id="zonaTrabajo" class="form-control" value="{{$query->zona_trabajo}}" name="zonaTrabajo" type="tel">                            
          </div>
          <div class="col-sm-2">
            <label for="telTrabajo" class="control-label">Tel. Trabajo</label>
            <input id="telTrabajo" class="form-control" value="{{$query->fax}}" name="telTrabajo" type="tel">                            
          </div>
          <div class="col-sm-3">
            ...
            <input type="hidden" id="idMunicipioTrabajo" name="idMunicipioTrabajo" value="{{$query->c_mpotrab}}">
          </div>
          <div class="col-sm-3">
            ...
            <input type="hidden" id="idDepartamentoTrabajo" name="idDepartamentoTrabajo" value="{{$query->c_deptotrab}}">
          </div>
        </div>
        <br>
        ...
        <div class="row">
          <div class="col-sm-2">
            ...
            <input id="fechaGraduacion" class="form-control" value="{{date('Y-m-d', strtotime($query->fecha_grad))}}" name="fechaGraduacion" type="date" max="9999-12-31">
          </div>
          <div class="col-sm-5">
            ...
            <input type="hidden" id="idUniversidadGraduado" name="idUniversidadGraduado" value="{{$query->c_universidad}}">
          </div>
          <div class="col-sm-5">
        ...
        <div class="row">
          <div class="col-sm-6">
            ...
            @else
            <input id="valUniversidadIncorporado" value="{{$uniinc->n_universidad}}" class="form-control ui-autocomplete-input" placeholder="Universidad" name="valUniversidadIncorporado" type="text" autocomplete="off">
            @endif
            <input type="hidden" id="idUniversidadIncorporado" name="idUniversidadIncorporado" value="{{$query->c_universidad1}}"> 
          </div>
          <div class="col-sm-6">
            <label for="tituloTesis" class="control-label">Título tesis</label>
            <input id="tituloTesis" class="form-control" value="{{$query->titulo_tesis}}" name="tituloTesis" type="text">                            
          </div>
        </div>
        <br>
        ...
        <div class="text-right m-t-15">
          ...
          <button id="ButtonColegiadoUpdate" class="btn btn-primary edit" type="submit">Actualizar Colegiado</button>
        </div>
      </div>
    </div>
  </div>
...
@endsection
@push('scripts')
<script src="{{asset('js/colegiados/editColegiado.js')}}"></script>
